...
export default function Dorsals() {
  ...
  return (
    <>
      {/* Barra de controls — oculta en imprimir */}
      <div className="no-print bg-gray-950 text-white px-6 py-4 border-b border-gray-800">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              className="bg-orange-500 hover:bg-orange-400 text-white font-bold px-5 py-2 rounded-lg text-sm transition"
            >
              Imprimir
            </button>
          </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">
            {bgImage ? 'Plantilla personalitzada activa' : 'Usant disseny per defecte · Puja una imatge per personalitzar'}
          </span>
        </div>
      </div>
      ...
      {carregant ? (
        ...
        <div className="dorsals-grid">
          {ciclistes.map(c => (
            <div
              key={c.id}
              className="dorsal-card"
              style={dorsalStyle}
            >
              {/* Disseny per defecte (visible si no hi ha imatge de fons) */}
              {!bgImage && (
                <>
              ...
              <div className="dorsal-header">
                <span>Tour de Charleys</span>
                <span>Torelló · 2025</span>
              </div>

              {/* Número */}
              <div className={`dorsal-numero ${bgImage ? 'dorsal-numero-shadow' : ''}`}>
                {c.numero_dorsal}
              </div>

              {/* Nom */}
              <div className={`dorsal-nom ${bgImage ? 'dorsal-nom-shadow' : ''}`}>
                {c.nom} {c.cognoms}
              </div>
            </div>
          ))}
